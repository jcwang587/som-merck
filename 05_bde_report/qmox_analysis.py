...
def get_job_spec_from_args(argv):
    ...
    @type argv: list
    @param argv: The list of command line arguments including the script name at
    [0], similar to that returned by sys.argv

    @rtype: L{launchapi.JobSpecification}
    @return: The JobSpecification for this job
    """

    parser, opts = parse_args(argv[1:])

    job_builder = launchapi.JobSpecificationArgsBuilder(argv,
                                                        use_jobname_log=True)
    job_builder.setProgramName(PROGRAM_NAME)

    if not job_builder.getJobname():
        job_builder.setJobname(DEFAULT_JOBNAME)

    # job_builder.setInputDirectory(str(opts.analysis_path_tar))
    job_builder.setInputFile(str(opts.analysis_path_tar))
    script_name = os.path.abspath(__file__)
    if script_name[-3:] == 'pyc':
        script_name = script_name[:-1]
    job_builder.setInputFile(script_name)
    # NOTE: Output file is added at runtime
    return job_builder.getJobSpec()

def parse_args(args):
    """
    Parser and validate given command-line options.
    """

    parser = get_parser()
    opts = parser.parse_args(args)

    analysis_path = opts.analysis_path_tar
    # grid_file = opts.grid_file

    # for filename in (in_poses_file, grid_file):
    if not analysis_path.is_file():
        parser.error('File not found: %s' % str(analysis_path))
...
class Analysis:
    '''Class to analyze Oxidation QM results'''

    def __init__(self, analysis_path_tar:Path, molname, cut_off_json, 
                o3_ene:float=-75.22035293792803,
                h_rad_ene:float=-0.49742380015,
                calcs:list=['sox', 'nox', 'cox'],
                mwfn_path:Path=Path('/apps/prod/DEVELOPMENT/MAS/qmox/Multiwfn'),
                alie_script:Path=Path('/apps/prod/DEVELOPMENT/MAS/qmox/alie_script.sh'),
                inkscape:Path=(
                    '/apps/prod/DEVELOPMENT/MAS/qmox/Inkscape-9c6d41e-x86_64.AppImage'),
                n_area:float=0,
                email:str='',
                outpath:Path=Path().resolve()) -> None:
        
        # self.untar_data(analysis_path_tar)
        self.analysis_path = Path().resolve() 
        self.molname = molname
        # Read json file
        # f = open(cut_off_json, 'r')
        f = open(cut_off_json, 'r')
        cut_off_dic = json.loads(f.read())
        # cut_off_dic = cut_off_json
        self.c_medium_coff = cut_off_dic['c_medium_coff']
        self.c_high_coff = cut_off_dic['c_high_coff']
        self.n_medium_coff = cut_off_dic['n_medium_coff']
        self.n_high_coff = cut_off_dic['n_high_coff']
        self.s_medium_coff = cut_off_dic['s_medium_coff']
        self.s_high_coff = cut_off_dic['s_high_coff']
        self.o3_ene = o3_ene
        self.h_rad_ene = h_rad_ene
        self.calcs = calcs
        self.mwfn_path = mwfn_path
        self.alie_script = alie_script
        self.inkscape = inkscape
        self.n_area = n_area
        self.email = email
        self.outpath = outpath
    
    def call_mwfn(self, files_path:Path):
        '''Function to call multiwfn and perform analysis'''
        with change_working_directory(files_path):
            # os.system(f"ssh inhhpclogin01 'bash -s' < {str(CHK2FCHK)} {str(files_path)}/")
            # subprocess.call(['sbatch', '--wait', str(MWFN_SH)])
            # subprocess.call(['sh', str(self.alie_script), str(self.mwfn_path)])
            result = subprocess.run(['sh', str(self.alie_script), str(self.mwfn_path), 
            str(IE_CMDS), str(EA_CMDS), str(AC_CMDS)], stdout=subprocess.PIPE, 
            stderr=subprocess.PIPE, universal_newlines=True)
            print(result.returncode, result.stderr, result.stdout)
            # self.copy_all_files(files_path, Path('/home/valdivig/Documents/Oxidation_pred/Maestro_script/test'))
            ProcessMwfn(files_path, area_coff=self.n_area).process_mwfn()

    def copy_all_files(self, orig_path:Path, dest_path:Path):
        for file in orig_path.iterdir():
            shutil.copy(str(file), str(dest_path / file.name))

    def untar_data(self, tar_file):
        subprocess.call(['tar', '-xf', str(tar_file)])

    def check_nox_logs(self, nox_path):

        incompletes = []    
        for out_file in nox_path.glob('*.out'):
            with open(out_file, 'r+') as f:
                lines = f.readlines()
            if not check_normal_termination(lines[-1]):
                incompletes.append(out_file)
        
        if incompletes:
            incompletes_path = Path(nox_path / 'notfinished_nox')
            incompletes_path.mkdir(exist_ok=True)

        for incomplete in incompletes:
            chk_file = incomplete.with_suffix('.chk')
            os.system(f'mv {str(chk_file)} {str(incompletes_path)}')



    def copy_files(self, root=str):
        '''root options: cox, sox, nox'''
        
        ox_path = self.analysis_path / root
        ox_path.mkdir(exist_ok=True)
        for file in self.analysis_path.glob(f'*_{root}*'):
            shutil.copy(str(file), str(ox_path / file.name))
            file.unlink()
        
        return ox_path

    def analyze_cox(self):
        
        cox_path = self.copy_files('cox')
        CoxidesAnalysis(cox_path, self.molname, self.c_medium_coff, self.c_high_coff, 
                        h_radical_ene=self.h_rad_ene, inkscape=self.inkscape, 
                        email=self.email, dir_report=self.outpath).analyze()
    
    def analyze_nox(self):
        
        nox_path = self.copy_files('nox_')
        print('Checking gaussian log files for termination errors')
        self.check_nox_logs(nox_path)
        chks_left = [i for i in nox_path.glob('*.chk')]
        if not chks_left:
            print('All nox calculations failed')
            send_email(self.email, f'{self.molname} N-oxidation calculations failed', 
            f'All N-oxides gaussian calculations failed for {self.molname}. Please chec'
            'k the gaussian out files.')
            return
        print('Calling mwfn')
        # self.call_mwfn(self.analysis_path)

        self.call_mwfn(nox_path)

        maegz = self.analysis_path / f'{self.molname}_confsearch-in.maegz'
        NoxidesAnalysis(maegz, nox_path, self.n_medium_coff, self.n_high_coff, 
                        sname=self.molname, inkspace=self.inkscape, 
                        email=self.email, out_dir=self.outpath).analyze()

    def analyze_sox(self):
        
        sox_path = self.copy_files('sox')
        SoxidesAnalysis(sox_path, self.s_medium_coff, 
                        self.s_high_coff, oxygen_h=self.o3_ene, inkscape=self.inkscape, 
                        sample=self.molname, email=self.email, 
                        dir_report=self.outpath).analyze()

        
    def analyze(self):
        '''
        Perform analysis based on job types found in self.analysis_path
        Sox has to be analyzed first as it might need nox data
        '''
        if 'sox' in self.calcs:
            self.analyze_sox()
        if 'cox' in self.calcs:
            self.analyze_cox()
        if 'nox' in self.calcs:
            self.analyze_nox()
...
def get_parser():
    ...
    parser.add_argument(
        'analysis_path_tar',
        help='Tar file with data to perform the analysis',
    ...
    parser.add_argument(
        '-calcs',
        help='Calculations to be analyzed. Options= cox, nox, sox',
        type=str,
        default='cox',
        )
    parser.add_argument(
        '-coffs',
        help='Json file with risk scale cut-offs values',
        type=lambda p:Path(p).resolve(),
        # default=COFF_DIC,
        )
    parser.add_argument(
        '-o3energy',
        help='Triplet oxygen energy in hartrees',
        type=float,
        default=O3_ENE,
        )
    parser.add_argument(
        '-henergy',
        help='Hydrogen radical energy in hartrees',
        type=float,
        default=H_RAD_ENE,
        )
    parser.add_argument(
    ...
    parser.add_argument(
        '-outpath',
        help='Outpath to send result files',
        type=lambda p:Path(p).resolve(),
        # default=COFF_DIC,
        )
    # parser.add_argument(
    ...
    # parser.add_argument(
    #     '-alie_script',
    #     help='Alie script path',
    #     type=lambda p:Path(p).resolve(),
    #     # default=COFF_DIC,
    #     )
    parser.add_argument(
        '-area_coff',
        help='N area cutoff value in A^2',
        type=float,
        # default=COFF_DIC,
        )
...
if __name__ == '__main__':
    ...
    # -JOBNAME options gets "swallowed" by $SCHRODINGER/run, so if we are
    # running under job control, we need to grab it from env var:
    ...
    logger.info('Calling Analysis class')
    Analysis(opts.analysis_path_tar, opts.sname, cut_off_json=opts.coffs, 
            o3_ene=opts.o3energy, h_rad_ene=opts.henergy, 
            calcs=opts.calcs, alie_script=ALIE_SH, mwfn_path=MWFN_SH,
            inkscape=INKSCAPE_DIR, n_area=opts.area_coff, email=opts.email,
            outpath=opts.outpath).analyze()

    # Analysis(Path().resolve(), 'BI425809', cut_off_dic=c_off_dic, o3_ene=O3_ENE,
    #         h_rad_ene=H_RAD_ENE, 
    #         calcs=[
    #             'cox', 
    #             # 'nox', 
